...
var page = {
  ...
  loadSearch: function () {
    ...
    _mock.getSearchResult(listParam, function (res) {
      _this.data = res;
      if(_this.data.data === null || _this.data.data === undefined || (_this.data.data instanceof String) || (typeof _this.data.data).toLowerCase() == 'string'){
        ...
        if($loadMoreWrap.hasClass('notshow')){ $loadMoreWrap.removeClass('notshow'); }
      }
  ...
  bindEvent : function(){
    ...
    $('#searchForm').on('submit', function (event) {
      ...
      _this.listParam.text = $.trim($('#searchInput').val());
      if(_this.listParam.text === ''){ return; }
      _this.loadSearch();
  });
  }
};
